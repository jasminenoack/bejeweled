...
view.prototype.dragDrop = function () {
  ...
  this.$ul.children().each(function (index, li) {
    ...
    $(li).droppable({
      drop: function (event, ui) {
        ...
        if (!that.disabled && that.game.validSwitch(ui.draggable.index(), $(this).index())) {
          that.disabled = true
          this.turns ++
  ...
  //       var $li = $(this)
  //       var pos = $li.index()
  //       var target = that.selected.index()
  //       $li.finish()
  //       that.selected.finish()
  //
  //       if ($li && that.selected && that.game.validSwitch(target, pos)) {
  //         that.turns ++
  //         that.selected.removeClass("selected")
  //         Bejeweled.Block.switchColors(that.selected, $li, that.checkBoard.bind(that))
  //         that.selected = null
  //       }
  //     },
  //     revert: true,
...
view.prototype.checkBoard = function () {
  // this.game.isTransitionsEnd($(".board li"),
  (function () {
...
view.prototype.timeBonus = function (score) {
  ...
  this.time = this.time + (2 / this.turns * score)
  if (this.time > 60 ) {
    this.time = 60
  }
  this.timerDraw()
