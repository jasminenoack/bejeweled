...
view.prototype.incrementScore = function(points) {
  if (points && points > 0) {
    stepBonus = Math.ceil((points - 3) / 3)

    if (!this.turnScore[this.turns]) {
      extraPoints = stepBonus
      this.turnScore[this.turns] = points + extraPoints
    } else {
      extraPoints = Math.ceil(points * (this.turnScore[this.turns] / 10)) + stepBonus
      this.turnScore[this.turns] = this.turnScore[this.turns] + extraPoints + points
    }

    this.score = this.score + extraPoints + points
    this.timeBonus(extraPoints)
    this.$notification.text("You scored " + (extraPoints + points) )
  }
}
